...
app.layout = dbc.Container(
    [dbc.Row(
     ...
     dbc.Row(
         html.Marquee("Get Daily Updated News About Covid 19 From Bahgeel Dashboard"), style={'color': 'red'}),
     dbc.Row([
         dbc.Col([
             dbc.Card(
                 [dbc.CardImg(src="https://media.tenor.com/NoO2NL5f5X8AAAAC/covid19-cell.gif", top=True, bottom=False),
                  dbc.CardBody([html.H4('COVID 19 DASHBOARD', className='card-title'),
                        html.P('Choose The Country:', className='card-text'),
                        dcc.Dropdown(id='selection_drop',
                                     multi=False,
                                     value='World',
                 ...
                 , color="dark"
                 , inverse=True,
                 outline=False)], width=2, xs=12, sm=12, md=12, lg=5, xl=2),
        ...
        dbc.Row([
            dbc.Col([
                dcc.Graph(id='indicator', figure={})], xs=4, sm=4, md=4, lg=2, xl=2),
            dbc.Col([dcc.Graph(id='mortality', figure={})],
                    xs=12, sm=12, md=12, lg=5, xl=5),
            dbc.Col([dcc.RadioItems(id='selection',
                                    options=['Trajectory of Pandemic',
                                             'Cumulative Cases'],
                                    value='Cumulative Cases'),
                     dcc.Loading(
                         dcc.Graph(id='graph', figure={}),
                                 type='cube')], xs=12, sm=12, md=12, lg=5,
                    xl=5)]),
     ], fluid=True)
...
def update_graph(selected_country):
    ...
    fig1 = go.Figure()
    fig1.add_scatter(name=f'Total Cases in {selected_country}', x=filtered_df['date'], y=filtered_df['total_cases'],
                     fill='tonexty', fillcolor='rgba(225,6,0,0.2)', line=dict(color='#e10600'))
    fig1.update_layout(title=f'Up to Date Covid 19 Cases in {selected_country} : Cumulative Figures',
...
def update_graph(selected_country):
    filtered_df = df[df['Country'] == selected_country]
    fig2 = go.Figure()
    fig2.add_scatter(name=f'New  Cases in {selected_country}', x=filtered_df['date'], y=filtered_df['new_cases'],
                     fill='tonexty', fillcolor='rgb(225,6,0)', line=dict(color='#e10600'))
    ...
    fig2.update_layout(title=f'The Trajectory of the Pandemic in {selected_country}', xaxis=dict(showgrid=False),
...
def update_graph(selected_country):
    filtered_df = df[df['Country'] == selected_country]
    fig3 = go.Figure()
    fig3.add_scatter(name=f'Mortality Rate in {selected_country}', x=filtered_df['date'],
                     y=filtered_df['Mortality Rate'], fill='tonexty', fillcolor='rgba(225,6,0,0.2)',
                     line=dict(color='#e10600'))
    fig3.add_scatter(name=f'Death Rate in {selected_country}', x=filtered_df['date'], y=filtered_df['Death Rate'],
                     line=dict(color='#FFFF00'))
    fig3.update_layout(title=f'Death Rate vs Mortalitly Rate in {selected_country}', xaxis=dict(showgrid=False),
                       yaxis=dict(showgrid=False), hovermode='x unified', paper_bgcolor='#000000',
                       plot_bgcolor='#000000')
...
def display_animated_graph(selection):
    if selection == 'Trajectory of Pandemic':
        ...
        fig.update_layout(transition={'duration': 1000}, paper_bgcolor='#000000', geo=dict(bgcolor='#000000'),
                          margin=dict(l=0, r=0, t=0, b=0))
        fig.update_traces(marker_line_color='rgba(255,255,255,0)', selector=dict(type='choroplethmapbox'))
        fig.layout.template = 'plotly_dark'
    else:
        df_country = pd.pivot_table(df, ('total_cases', 'new_cases', 'total_deaths'), index=['Countrycode', 'Country'],
